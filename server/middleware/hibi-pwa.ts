 ...
 * - `/__hibi/manifest.webmanifest` → per-app-named manifest (kept out of
 *   public/ so this dynamic response is the only one).
 * - Other HTML documents → stream-inject PWA + OG head tags at `</head>`.
 *   OG identity is baked via `virtual:hibi-og-identity` at `vite build`
...
import { hibiOgIdentity } from "virtual:hibi-og-identity";
import {
  acceptsHtml,
  createHeadInjector,
  isDocumentPath,
  ...
  renderWebManifest,
} from "../../scripts/hibi-pwa-shared.mjs";

interface HibiPwaEvent {
  url: URL;
  req: { method: string; headers: Headers };
}

function requestHost(event: HibiPwaEvent): string {
  return (
...
export default async function hibiPwaMiddleware(
  event: HibiPwaEvent,
  ...
  if (path === "/__hibi/manifest.webmanifest" || path === "/__hibi/manifest.json") {
